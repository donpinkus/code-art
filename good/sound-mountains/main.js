...
function renderFreq1() {
  ...
  for (var i = 0; i < spectrum.length; i++) {
    var x = map(i, 0, spectrum.length, 0, g.w + g.w * 0.1);
    var h = -g.h + map(spectrum[i], 0, 255, g.h, 0);

    rect(x, g.h, g.w / spectrum.length, h);
  }
}

function renderAmpCircle() {
  scale = map(amp.getLevel(), 0, 1.0, 10, g.w);

  // Draw the circle based on the volume
  fill(255, 224, 187);
  ellipse(g.w / 2, g.h / 2, scale, scale);
}
